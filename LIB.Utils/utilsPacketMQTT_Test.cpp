...
void UnitTest_PacketMQTT()
{
	...
	{
		tPacketCONNECT Pack(true, 10, "my_client_id", tQoS::AtLeastOnceDelivery, true, "my_will_topic", "my_will_message", "my_user_name", "my_password");
...

	{
		...
		tPacketCONNECT Pack(true, 10, "my_client_id345678901234567", tQoS::AtLeastOnceDelivery, true, "my_will_topic", "my_will_message", "my_user_name", "my_password");
		auto PackVector = Pack.ToVector();
		std::cout << utils::test::ToStringHEX(PackVector, true) << '\n';
		auto Pack_parsed = tPacketCONNECT::Parse(PackVector);
