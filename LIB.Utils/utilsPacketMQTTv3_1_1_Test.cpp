...
using namespace packet::mqtt_3_1_1;
using namespace packet::mqtt_3_1_1::hidden;
...
void UnitTest_PacketMQTTv3_1_1()
{
...

	{
		...
		utils::test::RESULT("TestPacket 1", PackType.has_value() && PackType->first == tControlPacketType::CONNECT);
	}
...

	{
		...
		[[maybe_unused]] auto pt1 = tPacketCONNECT::GetControlPacketType();
		[[maybe_unused]] auto pt2 = tPacketPUBLISH<tQoS::AtMostOnceDelivery>::GetControlPacketType();
		[[maybe_unused]] auto pt3 = tPacketNOACK::GetControlPacketType();

		[[maybe_unused]] tFixedHeaderBaseT<tControlPacketType::CONNECT> www;
		[[maybe_unused]] tFixedHeaderBaseT<tControlPacketType::PUBLISH> eee;
		//www.
		//eee.
	}

	{
		...
		utils::test::RESULT("Pack PUBLISH Parse 0x35", Pack.has_value() &&
			Pack->GetFixedHeader().GetQoS() == tQoS::ExactlyOnceDelivery &&
...

	{
		...
		utils::test::RESULT("Pack PUBLISH Parse 0x30", Pack.has_value() &&
			Pack->GetFixedHeader().GetQoS() == tQoS::AtMostOnceDelivery);
	}

	{// tQoS::AtMostOnceDelivery
		tPacketPUBLISH<tQoS::AtMostOnceDelivery> Pack(true, "demo_topic");
		...
		utils::test::RESULT("Response type", std::is_same<tPacketPUBLISH<tQoS::AtMostOnceDelivery>::response_type, tPacketNOACK>::value);
	}
	{// tQoS::AtMostOnceDeliver
		tPacketPUBLISH<tQoS::AtMostOnceDelivery> Pack(true, "demo_topic", { 1,2,3,4,5 });
		[[maybe_unused]] auto fh = Pack.GetFixedHeader();
		[[maybe_unused]] auto vh = Pack.GetVariableHeader();
		[[maybe_unused]] auto pl = Pack.GetPayload();
		UnitTest_PacketMQTT(Pack, "tPacketPUBLISH tQoS::AtMostOnceDelivery");
	...
	{// tQoS::AtLeastOnceDelivery
		tPacketPUBLISH<tQoS::AtLeastOnceDelivery> Pack(true, true, "demo_topic", 78);
		[[maybe_unused]] auto fh = Pack.GetFixedHeader();
		[[maybe_unused]] auto vh = Pack.GetVariableHeader();
		[[maybe_unused]] auto pl = Pack.GetPayload();
		UnitTest_PacketMQTT(Pack, "tPacketPUBLISH tQoS::AtLeastOnceDelivery");
		utils::test::RESULT("Response type", std::is_same<tPacketPUBLISH<tQoS::AtLeastOnceDelivery>::response_type, tPacketPUBACK>::value);
	}

	{// tQoS::AtLeastOnceDelivery
		tPacketPUBLISH<tQoS::AtLeastOnceDelivery> Pack(true, true, "demo_topic", 78, { 1,2,3,4,5 });
		[[maybe_unused]] auto fh = Pack.GetFixedHeader();
		[[maybe_unused]] auto vh = Pack.GetVariableHeader();
		[[maybe_unused]] auto pl = Pack.GetPayload();
		UnitTest_PacketMQTT(Pack, "tPacketPUBLISH tQoS::AtLeastOnceDelivery");
	...
	{// tQoS::ExactlyOnceDelivery
		tPacketPUBLISH<tQoS::ExactlyOnceDelivery> Pack(true, true, "demo_topic", 78);
		[[maybe_unused]] auto fh = Pack.GetFixedHeader();
		[[maybe_unused]] auto vh = Pack.GetVariableHeader();
		[[maybe_unused]] auto pl = Pack.GetPayload();
		UnitTest_PacketMQTT(Pack, "tPacketPUBLISH tQoS::ExactlyOnceDelivery");
		utils::test::RESULT("Response type", std::is_same<tPacketPUBLISH<tQoS::ExactlyOnceDelivery>::response_type, tPacketPUBREC>::value);
	}

	{// tQoS::ExactlyOnceDelivery
		tPacketPUBLISH<tQoS::ExactlyOnceDelivery> Pack(true, true, "demo_topic", 78, { 1,2,3,4,5 });
		[[maybe_unused]] auto fh = Pack.GetFixedHeader();
		[[maybe_unused]] auto vh = Pack.GetVariableHeader();
		[[maybe_unused]] auto pl = Pack.GetPayload();
		UnitTest_PacketMQTT(Pack, "tPacketPUBLISH tQoS::ExactlyOnceDelivery");
	...
	UnitTest_PacketMQTT(tPacketPUBLISH<tQoS::AtMostOnceDelivery>(true, "my_topic_name/some_device"), "Pack PUBLISH serialize-deserialize 2");
	...
	UnitTest_PacketMQTT(tPacketPUBLISH<tQoS::AtMostOnceDelivery>(true, "my_topic_name/some_device", { 0x01, 0x02 }), "Pack PUBLISH + Payload serialize-deserialize 2");
	...
	UnitTest_PacketMQTT(tPacketSUBSCRIBE(0x1234, { "my_topic_filter_1", tQoS::AtMostOnceDelivery }), "Pack SUBSCRIBE serialize-deserialize 2");
	{
...

	{
		...
		Filters.push_back("my_topic_filter_1");
		Filters.push_back("my_topic_filter_2");
		Filters.push_back("my_topic_filter_3");
		tPacketUNSUBSCRIBE Pack(0x1234, Filters);
		UnitTest_PacketMQTT(Pack, "Pack UNSUBSCRIBE serialize-deserialize 1");
	}
	UnitTest_PacketMQTT(tPacketUNSUBSCRIBE(0x1234,"my_topic_filter_1"), "Pack UNSUBSCRIBE serialize-deserialize 2");
